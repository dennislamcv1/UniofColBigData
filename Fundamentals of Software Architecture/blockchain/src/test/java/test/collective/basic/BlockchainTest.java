...
public class BlockchainTest {
    ...
    @Test
    public void testHashCalculation() throws NoSuchAlgorithmException {
        Block block = new Block("0", System.currentTimeMillis(), 0);
        String calculatedHash = block.calculatedHash();

        // The hash should be the same every time for the same inputs
        assertEquals(calculatedHash, block.getHash());
    }

    @Test
    public void testHashConsistency() throws NoSuchAlgorithmException {
        long timestamp = System.currentTimeMillis();
        Block block1 = new Block("0", timestamp, 0);
        Block block2 = new Block("0", timestamp, 0);

        // Different block objects with the same data should produce the same hash
        assertEquals(block1.getHash(), block2.getHash());
    }

    @Test
    public void testHashUniqueness() throws NoSuchAlgorithmException {
        Block block1 = new Block("0", System.currentTimeMillis(), 0);
        Block block2 = new Block("0", System.currentTimeMillis(), 1);

        // Blocks with different nonces should produce different hashes
